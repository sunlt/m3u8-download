...
const downloadSegments = async (m3u8Url, segments, output) => {
  const output_path = `${output}.tmp`
  try{
    fs.mkdirSync(output_path,{recursive: true})
  }catch(e){
    console.warn(e.message)
  }
  return Promise.map(segments, async (segment) => {    
    const myURL = new URL.URL(segment.uri, m3u8Url)
    const filePath = path.join(output_path,encodeURIComponent(segment.uri))
    return downloadSegment(myURL.href, filePath)
  },{concurrency: downloadThread})
}
...
const download = async (url, output) => {  
  ...
  const files = await downloadSegments(segmentsInfo.url, segments.slice(0,3), output)
  await merge2Ts(files, output)
  return output
}

const testUrl = 'https://meinv.jingyu-zuida.com/20200327/13051_9aab29e3/index.m3u8'
let Request = null
let downloadThread = 16

module.exports = ({url, output, proxy = null , thread = 16}) => {
  if(!url || !output){

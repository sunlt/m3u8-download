...
const downloadSegments = async (m3u8Url, segments, output) => {
  ...
  const parallelDownload = async () => {    
    ...
    const needDownloads = _.filter(segments, v => !v.downloading && !v.downloaded).slice(0,downloadThread - downloadings.length)

    await Promise.all(_.map(needDownloads,segment => {
